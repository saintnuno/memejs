...
test('utils.time', () => {
    const date1 = new Date(1608174423239); // 2020-12-17T03:07:03.239Z
    const date2 = new Date(1607408890.0 * 1000); // seconds

    expect(utils.time(Math.trunc(date1.valueOf() / 1000))).toBe('2020-12-17 03:07:03.000');
    expect(utils.time(Math.trunc(date2.valueOf() / 1000))).toBe('2020-12-08 06:28:10.000');
});
...
test('utils.subredditCheck - valid subredit', () => expect(utils.subredditCheck('https://www.reddit.com/r/puppy.json?sort=top&t=day&limit=100'))
    .resolves.toBe(true));

test('utils.subredditCheck - invalid url', () => expect(utils.subredditCheck('https://notautl.test'))
